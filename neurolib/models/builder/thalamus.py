...
# matrix as [from, to], masses as (TCR, TRN)
DEFAULT_THALAMIC_CONNECTIVITY = np.array([[0.0, 3.0], [5.0, 25.0]])
...
class ThalamicMass(NeuralMass):
    ...
    def _get_firing_rate(self, voltage):
        return self.parameters["Q_max"] / (
            1.0 + exp(-self.parameters["C1"] * (voltage - self.parameters["theta"]) / self.parameters["sigma"])
        )

    # synaptic currents
    def _get_excitatory_current(self, voltage, synaptic_rate):
        return self.parameters["g_AMPA"] * synaptic_rate * (voltage - self.parameters["E_AMPA"])

    def _get_inhibitory_current(self, voltage, synaptic_rate):
        return self.parameters["g_GABA"] * synaptic_rate * (voltage - self.parameters["E_GABA"])

    # intrinsic currents
    def _get_leak_current(self, voltage):
        return self.parameters["g_L"] * (voltage - self.parameters["E_L"])

    def _get_potassium_leak_current(self, voltage):
        return self.parameters["g_LK"] * (voltage - self.parameters["E_K"])
...
class ThalamocorticalPopulation(ThalamicMass):
    ...
    def __init__(self, parameters=None):
        super().__init__(parameters=parameters or DEFAULT_PARAMS_TCR)
    ...
    def _get_anomalous_rectifier_current(self, voltage, m_h1_value, m_h2_value):
        return (
            self.parameters["g_h"]
            * (m_h1_value + self.parameters["g_inc"] * m_h2_value)
            * (voltage - self.parameters["E_h"])
        )
    ...
    def _P_h(self, ca_conc):
        return (self.parameters["k1"] * ca_conc ** self.parameters["n_P"]) / (
            self.parameters["k1"] * ca_conc ** self.parameters["n_P"] + self.parameters["k2"]
        )
    ...
    def _derivatives(self, coupling_variables):
        ...
        ) / self.parameters["tau"] - (1.0 / self.parameters["C_m"]) * (
        ...
        d_ca_conc = (
            self.parameters["alpha_Ca"] * self._get_T_type_current(voltage, h_T)
            - (ca_conc - self.parameters["Ca_0"]) / self.parameters["tau_Ca"]
        )
        ...
        d_m_h1 = (
            ...
            - self.parameters["k3"] * self._P_h(ca_conc) * m_h1
            + self.parameters["k4"] * m_h2
        )
        d_m_h2 = self.parameters["k3"] * self._P_h(ca_conc) * m_h1 - self.parameters["k4"] * m_h2

        # synaptic dynamics
        d_syn_ext = dsyn_ext
        d_syn_inh = dsyn_inh
        d_dsyn_ext = (
            self.parameters["gamma_e"] ** 2
            * (
            ...
            - 2 * self.parameters["gamma_e"] * dsyn_ext
        )
        d_dsyn_inh = (
            self.parameters["gamma_r"] ** 2 * (coupling_variables["node_inh_exc"] - syn_inh)
            - 2 * self.parameters["gamma_r"] * dsyn_inh
        )
...
class ThalamicReticularPopulation(ThalamicMass):
    ...
    def __init__(self, parameters=None):
        super().__init__(parameters=parameters or DEFAULT_PARAMS_TRN)
    ...
    def _derivatives(self, coupling_variables):
        ...
        d_dsyn_inh = (
            self.parameters["gamma_r"] ** 2 * (coupling_variables["node_inh_inh"] - syn_inh)
            - 2 * self.parameters["gamma_r"] * dsyn_inh
        )
        # firing rate as dummy dynamical variable with infinitely fast
        # fixed-point dynamics
...
class ThalamicNetworkNode(SingleCouplingExcitatoryInhibitoryNode):
    ...
    def __init__(
        ...
        """
        :param tcr_parameters: parameters for the excitatory (TCR) mass
        :type tcr_parameters: dict|None
        :param trn_parameters: parameters for the inhibitory (TRN) mass
        :type trn_parameters: dict|None
        :param connectivity: local connectivity matrix
        :type connectivity: np.ndarray
        """
        tcr_mass = ThalamocorticalPopulation(parameters=tcr_parameters)
        tcr_mass.index = 0
        trn_mass = ThalamicReticularPopulation(parameters=trn_parameters)
        trn_mass.index = 1
        super().__init__(
