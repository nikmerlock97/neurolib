...
class WongWangMass(NeuralMass):
    ...
    def _get_firing_rate(self, current):
        return (self.parameters["a"] * current - self.parameters["b"]) / (
            1.0 - exp(-self.parameters["d"] * (self.parameters["a"] * current - self.parameters["b"]))
        )
...
class ExcitatoryWongWangMass(WongWangMass):
    ...
    def _derivatives(self, coupling_variables):
        ...
        current = (
            self.parameters["W"] * self.parameters["exc_current"]
            + coupling_variables["node_exc_exc"]
            + self.parameters["J"] * coupling_variables["network_exc_exc"]
            - coupling_variables["node_inh_exc"]
        )
        firing_rate_now = self._get_firing_rate(current)
        d_s = (
            (-s / self.parameters["tau"])
            + (1.0 - s) * self.parameters["gamma"] * firing_rate_now
...
class InhibitoryWongWangMass(WongWangMass):
    ...
    def _derivatives(self, coupling_variables):
        ...
        firing_rate_now = self._get_firing_rate(current)
        d_s = (-s / self.parameters["tau"]) + firing_rate_now + system_input(self.noise_input_idx[0])
        # firing rate as dummy dynamical variable with infinitely fast
        # fixed-point dynamics
        d_firing_rate = -self.parameters["lambda"] * (firing_rate - firing_rate_now)
...
class ReducedWongWangMass(WongWangMass):
    ...
    def __init__(self, parameters=None):
        super().__init__(parameters=parameters or DEFAULT_PARAMS_REDUCED)

    def _derivatives(self, coupling_variables):
        [s, firing_rate] = self._unwrap_state_vector()

        current = (
            self.parameters["w"] * self.parameters["J"] * s
            + self.parameters["J"] * coupling_variables["network_s"]
            + self.parameters["exc_current"]
        )
        firing_rate_now = self._get_firing_rate(current)
        d_s = (
            (-s / self.parameters["tau"])
...
class WongWangNetworkNode(SingleCouplingExcitatoryInhibitoryNode):
    ...
    def __init__(
        ...
        """
        excitatory_mass = ExcitatoryWongWangMass(exc_paramaters)
        excitatory_mass.index = 0
        inhibitory_mass = InhibitoryWongWangMass(inh_parameters)
...
class ReducedWongWangNetworkNode(Node):
    ...
    def __init__(self, parameters=None):
        """
        :param parameters: parameters of the reduced Wong-Wang mass
        :type parameters: dict|None
        """
        reduced_ww_mass = ReducedWongWangMass(parameters)
